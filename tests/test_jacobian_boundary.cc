// ---------------------------------------------------------------------
//
// Test boundary Jacobian against central finite differences.
//
// Assembles only the trace-boundary and RCR Jacobian blocks, then compares
// them against central finite differences of the corresponding residual.
//
// Note:
// The initial capacitor pressure Pc is initialized to the vessel reference
// pressure (p_d), not to the steady-state RCR value. Consequently, the
// initial RCR residual is generally nonzero.
//
// Output:
//   L2_error  -- ||J_an - J_fd||_F
//
// ---------------------------------------------------------------------
...
test()
{
  ...
  problem.assemble_jacobian_trace_boundary_block(t, problem.y_relevant);
  problem.assemble_jacobian_rcr_capacitor_block(problem.y_relevant);
  ...
  for (types::global_dof_index j = 0; j < problem.n_total_dofs; ++j)
    {
      ...
      Fp = 0.0;
      problem.assemble_trace_boundary_equations(t, problem.y_relevant, Fp);
      ...
      Fm = 0.0;
      problem.assemble_trace_boundary_equations(t, problem.y_relevant, Fm);
