...
#ifndef METRIC_FLOW_X_IO_VTK_UTILS_H
#define METRIC_FLOW_X_IO_VTK_UTILS_H
...
namespace MetricFlowX
{
  ...
  using dealii::Vector;
  namespace LinearAlgebra = dealii::LinearAlgebra;
   ...
   * geometry, cell and vertex data, mapping VTK fields to deal.II finite
   * elements, and transferring data between serial and distributed
   * representations.
   *
   ...
   * - Translating VTK data arrays into deal.II vectors associated with
   * DoFHandler objects.
   ...
   */
  {
    ...
    /**
     * Helper to extract a single double value from the raw VTK cell data
     * vector.
     */
    inline double
    get_cell_value(const unsigned int              cell_index,
                   ...
                   const std::vector<std::string> &names)
    {
      ...
      if (it == names.end())
        return 0.0;
      const unsigned int col = std::distance(names.begin(), it);
      // The data vector is stored as [cell0_val0, cell0_val1, ..., cell1_val0,
      // ...]
      return data(cell_index * names.size() + col);
    }

    /**
     * Helper to extract a single double value from the raw VTK point data
     * vector.
     */
    inline double
    get_point_value(const unsigned int              point_index,
                    const std::string              &name,
                    const Vector<double>           &data,
                    const std::vector<std::string> &names)
    {
      ...
      if (it == names.end())
        return 0.0;
      const unsigned int col = std::distance(names.begin(), it);
      // Point data is stored similarly to cell data
      return data(point_index * names.size() + col);
    }
     ...
     *
     * This function returns a BlockIndices object that represents the mapping
     * of components of the finite element to blocks for the specified finite
     * element.
     *
    ...
    /**
     * Translate a vtk data file (obtained through read_data()) to a dealii
     * vector type, associated with the given DoFHandler object.
     *
     ...
     *
     * The DoFHandler must be already initialized with the finite element
     * obtained through the vtk_to_finite_element() method. The grid underlying
     * the DoFHandler dh must be generated by the serial_tria for this method to
     * work.
     *
     * @tparam dim
     * @tparam spacedim
     ...
     * that the serial vector contains data for all vertices in the mesh, and
     * that the global cell indices are preserved in the distributed vector.
     *
     ...
     *
     * The parallel Triangulation must have been generated from the serial one
     * for this function to be any meaningful at all.
     *
...
#  ifndef DOXYGEN
                          ...
                          VectorType                         &output_vector)
    {
      ...
      const auto dist_to_serial_vertices =
        distributed_to_serial_vertex_indices(serial_tria,
      ...
      for (unsigned int field = 0; field < fe.n_blocks(); ++field)
        {
          ...
          if (field_fe.n_dofs_per_vertex() > 0)
          ...
          else if (field_fe.template n_dofs_per_object<dim>() > 0)
            {
              ...
              // Assumption: serial and parallel meshes have the same ordering
              // of cells.
              ...
              for (; parallel_cell != dh.end(); ++parallel_cell)
                if (parallel_cell->is_locally_owned())
                  {
                    // Advanced serial cell until we reach the same cell index
                    // of the parallel cell
...
#endif // METRIC_FLOW_X_IO_VTK_UTILS_H
