    ...
    for (unsigned int cycle = 0; cycle < n_refinement_cycles; ++cycle)
      {
        ...
        // -------------------------------- Central differences along a random
        // direction.  Everything here is a collective distributed operation:
        // the direction is seeded per rank but only its owned entries are
        // filled, and the norms and the worst row are reduced across the
        // communicator.
        auto check_jacobian_fd = [this](const VectorType &y0) {
          ...
          VectorType ydot0(locally_owned_dofs_, mpi_communicator_);
          ydot0 = 0.0;

          assemble_jacobian(t, y0, ydot0, alpha);

          VectorType v(locally_owned_dofs_, mpi_communicator_);
          std::srand(this_mpi_process + 1);
          for (const auto i : locally_owned_dofs_)
          ...
          VectorType Fp(locally_owned_dofs_, mpi_communicator_);
          VectorType Fm(locally_owned_dofs_, mpi_communicator_);
          ...
          VectorType Jv(locally_owned_dofs_, mpi_communicator_);
          ...
          const double worst =
            Utilities::MPI::max(worst_local, mpi_communicator_);
        ...
        SUNDIALS::IDA<VectorType> ida(ida_parameters, mpi_communicator_);

        ida.reinit_vector = [this](VectorType &v) {
          v.reinit(locally_owned_dofs_, mpi_communicator_);
        };
        ...
        ida.output_step = [this](const double      t,
          ...
          time = t;
          compute_pressure(sol, pressure_);
          output_results(sol, pressure_, step_number);
          write_csv_row(t, sol);
        };
        ...
        close_csv_files();
        compute_pressure(solution, pressure_);
