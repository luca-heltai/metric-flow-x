#!/usr/bin/env bash
# Run the source-backed 37-segment arterial network tutorial after validating all local inputs.
set -euo pipefail
...
if grep -q '@SOURCE_DIR@' "$PARAMETER_INPUT"; then
  TEMP_PARAMETER=$(mktemp "${TMPDIR:-/tmp}/37-segment-arterial-network-parameters.XXXXXX.prm")
...
mkdir -p "$ROOT_DIR/output/37_segment_arterial_network"
cd "$ROOT_DIR"
echo "Running 37-segment arterial network tutorial with $RESOLVED_PARAMETER"
